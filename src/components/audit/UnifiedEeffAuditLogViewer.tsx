...
interface UnifiedEeffAuditLogViewerProps {
  ruc: string;
}

const UnifiedEeffAuditLogViewer: React.FC<UnifiedEeffAuditLogViewerProps> = ({ ruc }) => {
  const loadLogs = async (): Promise<BaseAuditLog[]> => {
    if (!ruc) {
      return [];
    }
    const data = await EeffAuditLogService.getLogsByRuc(ruc);
  ...
  return (
    <AuditLogViewerComponent
      loadLogs={loadLogs}
      trigger={trigger}
      title={`Historial Unificado de EEFF para RUC: ${ruc}`}
      description="Registro completo de todos los cambios realizados en los estados financieros de esta empresa, a través de los años."
      groupLogs={true}
...
export default UnifiedEeffAuditLogViewer;