...
interface GroupedLog {
  ...
  created_at: string;
  logs: RibReporteTributarioAuditLogWithUserInfo[];
}
...
const RibReporteTributarioAuditLogViewer: React.FC<RibReporteTributarioAuditLogViewerProps> = ({ reporteId }) => {
  ...
  const groupLogsByOperation = (logs: RibReporteTributarioAuditLogWithUserInfo[]) => {
    const groups: GroupedLog[] = [];
    
    logs.forEach((log) => {
      // Buscar si existe un grupo con el mismo timestamp (dentro de 5 segundos) y mismo usuario
      const existingGroup = groups.find((group) => {
        const timeDiff = Math.abs(
          new Date(group.created_at).getTime() - new Date(log.created_at).getTime()
        );
        return (
          timeDiff < 5000 && // Dentro de 5 segundos
                ...
                <CardContent className="pt-4">
                      ...
                      <div>
                        ...
                        {group.logs.length > 1 && (
                          <span className="ml-2 text-xs text-gray-400">
                            ({group.logs.length} años actualizados)
                          </span>
                        )}
                      </div>
                    </div>
                    ...
                    <>
                      ...
                      <div className="space-y-2">
                        ...
                        {group.logs.map((log, logIndex) => {
                          ...
                          // Obtener el año del log (está en old_values o new_values)
                            ...
                            <div key={logIndex}>
                              {Object.keys(log.changed_fields)
                                .filter(field => {
                                  ...
                                  // Ignorar campos desconocidos o que no tienen valores
                                  const oldValue = log.old_values?.[field];
                                  const newValue = log.new_values?.[field];
                                  
                                  // Si ambos son undefined/null, ignorar
                  ...
                  {group.action === 'created' && (
                    <div className="mt-3 text-sm text-gray-400">
                      <p>✨ Reporte RIB creado inicialmente</p>
                    </div>
                  )}
                </CardContent>
              </Card>
