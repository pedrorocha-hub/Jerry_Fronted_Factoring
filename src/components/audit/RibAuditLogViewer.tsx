...
import { RibAuditLogWithUserInfo, RibAuditAction } from '@/types/rib-audit-log';
...
const RibAuditLogViewer: React.FC<RibAuditLogViewerProps> = ({ ribId }) => {
  const [logs, setLogs] = useState<RibAuditLogWithUserInfo[]>([]);
  ...
  const loadLogs = async () => {
    ...
    try {
      ...
      console.error('Error loading RIB audit logs:', error);
    } finally {
      setLoading(false);
    }
  };

  const getActionIcon = (action: RibAuditAction) => {
    switch (action) {
      case 'created':
  ...
  const getActionLabel = (action: RibAuditAction) => {
    switch (action) {
      case 'created':
        return 'Creado';
      case 'updated':
  ...
  const getActionBadgeColor = (action: RibAuditAction) => {
    switch (action) {
      case 'created':
  ...
  const formatFieldName = (field: string): string => {
    const fieldNames: Record<string, string> = {
      ruc: 'RUC',
      direccion: 'Dirección',
      como_llego_lcp: '¿Cómo llegó a LCP?',
      telefono: 'Teléfono',
      ...
      relacion_comercial_deudor: 'Relación Comercial con el Deudor',
      ...
      <DialogContent className="bg-[#121212] border-gray-800 text-white max-w-4xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-2xl flex items-center">
            ...
            Historial de Auditoría - Análisis RIB
          </DialogTitle>
          <p className="text-gray-400 text-sm">
            Registro completo de todos los cambios realizados en este análisis
          </p>
        </DialogHeader>
        ...
        {loading ? (
          ...
          <div className="text-center py-8 text-gray-400">
            ...
            <p>No hay cambios registrados para este análisis.</p>
          </div>
        ) : (
                ...
                <CardContent className="pt-4">
                  ...
                  {log.action === 'created' && (
                    <div className="mt-3 text-sm text-gray-400">
                      <p>✨ Análisis RIB creado inicialmente</p>
                    </div>
                  )}
                </CardContent>
              </Card>
