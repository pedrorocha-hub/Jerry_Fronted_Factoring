...
const AccionistaManager: React.FC<AccionistaManagerProps> = ({ ruc }) => {
          ...
          <DialogContent className="bg-[#121212] border border-gray-800 max-w-2xl">
            <DialogHeader>
              <DialogTitle className="text-white">
                {editingAccionista ? 'Editar Accionista' : 'Nuevo Accionista'}
              </DialogTitle>
            </DialogHeader>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input value={formData.dni} onChange={(e) => setFormData(prev => ({ ...prev, dni: e.target.value }))} required className="bg-gray-900/50 border-gray-700 text-white" />
                </div>
                <div>
                  <Label className="text-gray-300">Nombre Completo</Label>
                  <Input value={formData.nombre} onChange={(e) => setFormData(prev => ({ ...prev, nombre: e.target.value }))} required className="bg-gray-900/50 border-gray-700 text-white" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label className="text-gray-300">Porcentaje (%)</Label>
                  <Input type="number" step="0.01" min="0" max="100" value={formData.porcentaje} onChange={(e) => setFormData(prev => ({ ...prev, porcentaje: parseFloat(e.target.value) || 0 }))} className="bg-gray-900/50 border-gray-700 text-white" />
                </div>
                <div>
                  <Label className="text-gray-300">Vínculo</Label>
                  <Input value={formData.vinculo} onChange={(e) => setFormData(prev => ({ ...prev, vinculo: e.target.value }))} className="bg-gray-900/50 border-gray-700 text-white" />
                </div>
              </div>
              <div>
                <Label className="text-gray-300">Calificación</Label>
                <Input value={formData.calificacion} onChange={(e) => setFormData(prev => ({ ...prev, calificacion: e.target.value }))} className="bg-gray-900/50 border-gray-700 text-white" />
              </div>
              <div>
                <Label className="text-gray-300">Comentario</Label>
                <Textarea value={formData.comentario} onChange={(e) => setFormData(prev => ({ ...prev, comentario: e.target.value }))} className="bg-gray-900/50 border-gray-700 text-white" />
              </div>
              <div className="flex justify-end space-x-2 pt-4">
                ...
                <Button type="submit" className="bg-[#00FF80] hover:bg-[#00FF80]/90 text-black">{editingAccionista ? 'Actualizar' : 'Crear'}</Button>
              </div>
            </form>
          </DialogContent>
        </Dialog>
                ...
                accionistas.map(acc => (
                  <TableRow key={acc.id} className="border-gray-800">
                    ...
                    <TableCell className="text-right">
                      <Button variant="ghost" size="sm" onClick={() => handleEdit(acc)} className="text-gray-400 hover:text-[#00FF80]"><Edit className="h-4 w-4" /></Button>
                      <AlertDialog>
