...
interface ComentariosEjecutivoFormProps {
  comentario?: ComentarioEjecutivo | null;
  solicitudId: string;
  onSave?: (comentario: ComentarioEjecutivo) => void;
  disabled?: boolean;
}

const ComentariosEjecutivoForm: React.FC<ComentariosEjecutivoFormProps> = ({
  comentario,
  solicitudId,
  onSave,
  disabled = false
  ...
  // Inicializar datos del comentario existente
  useEffect(() => {
    if (comentario) {
      setComentarioText(comentario.comentario);
  ...
  const handleSave = async () => {
    ...
    if (!solicitudId) {
      setError('Solicitud de operación requerida');
      return;
    }
    ...
    try {
      ...
      const comentarioData: ComentarioEjecutivo = {
        id: comentario?.id,
      ...
      <CardContent className="space-y-4">
        ...
        <div>
          ...
          <Textarea
            ...
            placeholder="Ingrese sus comentarios sobre la solicitud de operación..."
        ...
        <div>
          ...
          >
            ...
            <p className="text-sm text-gray-500">
              PDF, DOC, DOCX, PNG, JPG, JPEG, GIF, WEBP, TXT (máx. 10MB cada uno)
            </p>
          </div>
        ...
        <div className="flex justify-end pt-4">
          <Button
            onClick={handleSave}
            disabled={saving || disabled || !comentarioText.trim() || !solicitudId}
...
export default ComentariosEjecutivoForm;
