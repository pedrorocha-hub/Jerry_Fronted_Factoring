import React from 'react';
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from '@/components/ui/table';
import { Button } from '@/components/ui/button';
import { Edit, Trash2, Download, Calendar } from 'lucide-react';
import { SolicitudOperacion } from '@/types/solicitud-operacion';
import { useSession } from '@/contexts/SessionContext';

interface SolicitudOperacionWithDetails extends SolicitudOperacion {
  nombre_empresa?: string;
  creator_name?: string;
}

interface SolicitudOperacionTableProps {
  solicitudes: SolicitudOperacionWithDetails[];
  onEdit: (solicitud: SolicitudOperacion) => void;
  onDelete: (solicitud: SolicitudOperacion) => void;
  onDownload: (solicitud: SolicitudOperacion) => void;
}

const getStatusColor = (status: string | null | undefined) => {
  switch (status) {
    case 'Completado':
      return 'bg-green-500/20 text-green-400';
    case 'En revisión':
      return 'bg-yellow-500/20 text-yellow-400';
    case 'Borrador':
    case 'draft':
    default:
      return 'bg-gray-500/20 text-gray-400';
  }
};

const SolicitudOperacionTable: React.FC<SolicitudOperacionTableProps> = ({ solicitudes, onEdit, onDelete, onDownload }) => {
  const { isAdmin } = useSession();

  return (
    <div className="overflow-x-auto">
      <Table>
        <TableHeader>
          <TableRow className="border-gray-800 hover:bg-gray-900/50">
            <TableHead className="text-gray-300">Empresa</TableHead>
            <TableHead className="text-gray-300">Creador</TableHead>
            <TableHead className="text-gray-300">Estado</TableHead>
            <TableHead className="text-right text-gray-300">Acciones</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {solicitudes.map((solicitud) => (
            <TableRow key={solicitud.id} className="border-gray-800 hover:bg-gray-900/50">
              <TableCell>
                <div className="font-medium text-white">{solicitud.nombre_empresa}</div>
                <div className="text-sm text-gray-400 font-mono">{solicitud.ruc}</div>
              </TableCell>
              <TableCell>
                <div className="text-sm text-gray-300">{solicitud.creator_name}</div>
                <div className="flex items-center text-xs text-gray-500 mt-1">
                  <Calendar className="h-3 w-3 mr-1.5" />
                  {new Date(solicitud.created_at).toLocaleDateString('es-ES')}
                </div>
              </TableCell>
              <TableCell>
                <span className={`px-2 py-1 text-xs rounded-full capitalize ${getStatusColor(solicitud.status)}`}>
                  {solicitud.status || 'Borrador'}
                </span>
              </TableCell>
              <TableCell className="text-right">
                <Button variant="ghost" size="icon" onClick={() => onDownload(solicitud)} className="text-gray-400 hover:text-white">
                  <Download className="h-4 w-4" />
                </Button>
                {isAdmin && (
                  <>
                    <Button variant="ghost" size="icon" onClick={() => onEdit(solicitud)} className="text-gray-400 hover:text-white">
                      <Edit className="h-4 w-4" />
                    </Button>
                    <Button variant="ghost" size="icon" onClick={() => onDelete(solicitud)} className="text-gray-400 hover:text-red-500">
                      <Trash2 className="h-4 w-4" />
                    </Button>
                  </>
                )}
              </TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
    </div>
  );
};

export default SolicitudOperacionTable;