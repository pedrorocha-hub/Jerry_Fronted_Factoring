...
const SolicitudOperacionCreateEditPage = () => {
  ...
  const hasRelacion = !!(solicitudFormData.antiguedad_vinculo || solicitudFormData.volumen_estimado || solicitudFormData.condicion_pago_dias);
  const hasExperiencia = !!(solicitudFormData.experiencia_lcp || solicitudFormData.check_pagos_observados);
  const hasRiesgoProv = riesgoRows.some(r => r.riesgo_aprobado || r.propuesta_comercial);
  const hasRiesgoDeudor = !!(top10kData || solicitudFormData.deudor_ruc);
  const hasContacto = !!(solicitudFormData.direccion || solicitudFormData.visita_contacto_nombre || solicitudFormData.visita_fecha);
